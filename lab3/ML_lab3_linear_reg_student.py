...
def parsePoint(line):
    ...
    data= LabeledPoint(label,features)
    return data

parsedSamplePoints = parsePoint('2001.0,0.8841,0.6105,0.6005,0.4747,0.2472,0.3573,0.3441,0.3396,0.6009,0.4257,0.6049,0.4192')
...
onlyLabels = parsedDataInit.map(lambda y:y.label)
print type(parsedDataInit)
...
for alpha in [1e-5,10]:
    for numIters in [500,5]:
        model = LinearRegressionWithSGD.train(parsedTrainData, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType=regType, intercept=True)
        labelsAndPreds = parsedValData.map(lambda lp: (lp.label, model.predict(lp.features)))
...
def twoWayInteractions(lp):
    ...
    """
    tmp=itertools.product(range(len(lp.features)),range(len(lp.features)))
    return LabeledPoint(lp.label,np.hstack((lp.features,[lp.features[i]*lp.features[j] for (i,j) in tmp])))
